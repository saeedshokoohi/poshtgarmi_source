...
var replaceDay = function(str, date) {
  ...
  if (match) {
    ...
    var replaceHour = function(str, date) {
        match = str.match(/[hH]+/);
        if (match) {
            switch (match[0]) {
                case 'H':
                case 'HH':
                    var value = replaceHour(str.replace(match, date.getHours()), date);
                    return value;
            }
        } else {
            return str;
    ...
    var replaceMinute = function(str, date) {
       return str.replace('SS',date.getMinutes());
  ...
  */
function JDate(_date,m,d,h,mi,s) {
  var date, _d;

  if(m!=undefined && d!=undefined  )
  {
	    var tempDate=new Date(_date,m,d,h,mi,s);
	  if(tempDate instanceof Date && tempDate.getDate()>0)
	  {
	  this._d=tempDate;
	  ...
	  {
		  var tempDate=[];
		  tempDate.push(parseInt(_date));
		  tempDate.push(parseInt(m));
		  tempDate.push(parseInt(d));
		  this.date=tempDate;
  ...
  else if(typeof this._d==='number')
  {
	  this._d=new Date(this._d);
	   this.date = JDate.to_jalali(this._d);
  }
...
JDate.prototype.setHours=function (hr)
{
	this._d.setHours(hr);
}
JDate.prototype.setMinutes =function (mi)
{
	this._d.setMinutes(mi);
}
JDate.prototype.setSeconds  =function (s)
{
	this._d.setSeconds(s);
}
JDate.prototype.setMilliseconds   =function (ms)
{
...
JDate.prototype.getTimezoneOffset =function ()
{
	return this._d.getTimezoneOffset();
}
JDate.prototype.getTime=function()
{
	return this._d.getTime();
}
JDate.prototype.setTime=function(t)
{
	return this._d.setTime(t);
}
JDate.prototype.valueOf=function()
{
	return this._d.valueOf();
}
JDate.prototype.ticksOf=function()
{
	return this._d.valueOf();
}
/*
...
//	debugger;
	var maxDaysOfMonth=JDate.daysInMonth(this.date[0],this.date[1]);
  if(date<1)
  {	  this.date[2]=1;
      this._d = this.to_gregorian();
  ...
  }else if(date>maxDaysOfMonth)
  {
	  this.date[2]=maxDaysOfMonth;
      this._d = this.to_gregorian();
	  var dd=this._d.getDate();
	  dd=dd+(date-maxDaysOfMonth);
	  this._d.setDate(dd);
	   this.date = JDate.to_jalali(this._d);
  }else
  {
...
JDate.prototype.format = function(format) {
  ...
  format = replaceHour(format, this);
  format = replaceMinute(format, this);
  return format;
}

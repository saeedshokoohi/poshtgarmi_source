...
dtmlXMLLoaderObject.prototype.loadXML=function(filePath, postMode, postVars, rpc){
    if (this.rSeed)
        filePath+=((filePath.indexOf("?") != -1) ? "&" : "?")+"a_dhx_rSeed="+(new JDate()).valueOf();
...
dhtmlDragAndDropObject.prototype.preCreateDragCopy=function(e){
    ...
    window.dhtmlDragAndDrop.downtime = new JDate().valueOf();
...
dhtmlDragAndDropObject.prototype.callDrag=function(e){
    ...
    if ((new JDate()).valueOf()-dragger.downtime<100) return;
...
dataProcessor.prototype={
	...
	*/
    ...
    _sendData:function(a1,rowId){
    	...
    	if (rowId)
			this._in_progress[rowId]=(new JDate()).valueOf();
	...
	_getAllData:function(rowId){
		...
		for(var i=0;i<this.updatedRows.length;i++){
			...
			has_one = true;
			this._in_progress[id]=(new JDate()).valueOf();
		}
...
*/
	...
	*/
	setAutoUpdate: function(interval, user) {
		...
		this._user = user || (new JDate()).valueOf();
...
scheduler.getState=function(){
	return {
		...
		min_date: new  JDate(this._min_date),
		max_date: new  JDate(this._max_date),
...
scheduler._click={
	dhx_cal_data:function(e){
		...
		if (id && scheduler.config.select) {
			...
			scheduler._close_not_saved();
			if (new JDate().valueOf()-(scheduler._new_event||0) > 500){
				scheduler.unselect();
			}
		}
	},
...
scheduler.addEventNow=function(start,end,e){
	...
	var start_date = new  JDate(start);
	if (!end){
	...
	var end_date = new  JDate(end);

	// scheduler.addEventNow(new JDate(), new JDate()) + collision though get_visible events defect (such event was not retrieved)
...
scheduler._get_column_index = function(x_pos){
    ...
    //by saeed
    //to revert colmn index of week by mouse left
    return 7-column;
	//return column;
};
...
scheduler._mouse_coords=function(ev){
    ...
    pos.x-=getAbsoluteLeft(this._obj);
...
scheduler._close_not_saved=function(){
	if (new JDate().valueOf()-(scheduler._new_event||0) > 500 && scheduler._edit_id){
...
scheduler._correct_shift=function(start, back){
    return start;
    //this line commented by saeed
    //bcz it couses wrong start date
//	return start-=((new  JDate(scheduler._min_date)).getTimezoneOffset()-(new  JDate(start)).getTimezoneOffset())*60000*(back?-1:1);
};
...
scheduler._is_pos_changed = function(old_pos, new_pos){
	...
	return !!(!this._drag_pos.start || (+new JDate() - this._drag_pos.start > delay) || diff(old_pos.x, new_pos.x, d_pos) || diff(old_pos.y, new_pos.y, d_pos));
};
...
scheduler._on_mouse_move=function(e){
	if (this._drag_mode){
		...
		if (pos.force_redraw || this._is_pos_changed(this._drag_pos, pos)){
				...
				if (this._table_view) {
					...
					if (this._mode == "month") {
						...
						if( this._drag_from_start ) {
							var day = 24*60*60000;
							if( resize_date <= scheduler.date.date_part(new  JDate(end+day-1)).valueOf() ) // to get end time as 23:59:59 and then the day start
					...
					end = this.date.date_part(new  JDate(ev.end_date.valueOf() - 1)).valueOf()+pos.y*this.config.time_step*60000;
			...
			var new_end = new  JDate(end-1);
			var new_start = new  JDate(start);
			...
			if(scheduler.config.limit_drag_out &&
				(+new_start < +scheduler._min_date || +end > +scheduler._max_date)){
					...
					if(+new_start < +scheduler._min_date){
						new_start = new  JDate(scheduler._min_date);
						end = new  JDate(+new_start + duration);
					}else{
						end = new  JDate(scheduler._max_date);
						new_start = new  JDate(+end - duration);
					}
					var new_end = new  JDate(end-1);
			}
			...
			if(!this._table_view &&
				...
				((!scheduler._get_section_view() && pos.x != this._get_event_sday({start_date: new  JDate(end), end_date:new  JDate(end)})) || new  JDate(end).getHours() >= this.config.last_hour)){
				var duration = end - new_start;
				var day = this._min_date.valueOf()+(pos.x*24*60)*60000;
				end = scheduler.date.date_part(new  JDate(day));
				end.setHours(this.config.last_hour);
				new_end = new  JDate(end-1);
				if(this._drag_mode == "move"){
					new_start = new  JDate(+end - duration);
				}
			}
...
scheduler._on_mouse_down=function(e,src) {
	...
	if (this._drag_mode){
		...
		if (!this.config["drag_"+this._drag_mode] || !this.callEvent("onBeforeDrag",[id, this._drag_mode, e]))
			...
			this._drag_pos.start = +new JDate();
		}
	}
		...
		if (moved && (this._drag_event._dhx_changed || !this._drag_event.start_date || ev.start_date.valueOf()!=this._drag_event.start_date.valueOf() || ev.end_date.valueOf()!=this._drag_event.end_date.valueOf())){
			...
			if (!this.callEvent("onBeforeEventChanged",[ev, e, is_new, this._drag_event])){
				...
				if (is_new && this.config.edit_on_create){
					this.unselect();
					this._new_event=new JDate();//timestamp of creation
...
scheduler._get_timeunit_start = function(){
	...
	return this.date[this._mode+"_start"](new JDate(this._date.valueOf()));
};
...
scheduler._reset_hours_scale=function(b,dd,sd){
	...
	var date = new  JDate(1980,1,1,this.config.first_hour,0,0);
...
scheduler._currentDate = function(){
	if(scheduler.config.now_date){
		return new  JDate(scheduler.config.now_date);
	}
	return new JDate();
};
...
//debugger;
	...
	for (var i=0; i<rows; i++){
		...
		for (var j=0; j<7; j++) {
			...
			if (sd.getDate() - bf1 > 1)
				sd = new  JDate(sd.getFullYear(), sd.getMonth(), bf1 + 1, 12, 0);
		}
		html+="</tr>";
...
scheduler._lame_clone = function(object, cache) {
	...
	if (object && typeof object == "object") {
		result = {};
		t = [Array,JDate,Number,String,Boolean];
...
scheduler._get_date_from_pos = function(pos) {
	...
	return new  JDate(this._correct_shift(start));
};
// n_ev - native event
...
scheduler._get_fictional_event_length=function(end_date, ev_length, obj, back){
	...
	var fd = new  JDate(end_date*1+ev_length*dir);
	var ignore = this["ignore_"+this._mode];
...
scheduler.date={
	...
	week_start:function(date){
        ...
        //this line added by saeed
        //to start week from saturday
        shift=shift+1;
        //end edit
	...
	_add_days:function(date, inc){
		var ndate = new  JDate(date.valueOf());
	...
	str_to_date:function(format,utc){
		...
		return new Function("date","var set=[0,0,1,0,0,0]; "+splt+" return new  JDate("+code+");");
	},
	getISOWeek: function(ndate) {
		...
		var first_thursday = new  JDate(ndate.valueOf());
		first_thursday.setDate(ndate.getDate() + (4 - nday));
		var year_number = first_thursday.getFullYear(); // year of the first Thursday
		var ordinal_date = Math.round( (first_thursday.getTime() - new  JDate(year_number, 0, 1).getTime()) / 86400000); //ordinal date of the first Thursday - 1 (so not really ordinal date)
	...
	convert_to_utc: function(date) {
		return new  JDate(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds());
	}
};
scheduler.locale = {
	date:{
...
scheduler.uid = function() {
	if (!this._seed) this._seed = (new JDate()).valueOf();
	return this._seed++;
};
scheduler._events = {};
...
scheduler._copy_dummy = function() {
	var a = new  JDate(this.start_date);
	var b = new  JDate(this.end_date);
	this.start_date = a;
	this.end_date = b;
};
...
scheduler.render_event = function(ev) {
	...
	left = left + parseInt(parent.style.right, 10) + menu_offset;
...
scheduler._render_v_bar = function (ev, x, y, w, h, style, contentA, contentB, bottom) {
	...
	var html = '<div event_id="' + id + '" class="' + cs + '" style="position:absolute; top:' + y + 'px; right:' + x + 'px; z-index: 10000; width:' + (w - 4) + 'px; height:' + h + 'px;' + (style || "") + '"></div>';
	d.innerHTML = html;
...
scheduler.getEvents = function(from, to) {
	...
	for (var a in this._events) {
		...
		if (ev && ( (!from && !to) || (ev.start_date < to && ev.end_date > from) )) {
            if(ev.start_date instanceof JDate)
            ev.start_date=ev.start_date._d;
            if(ev.end_date instanceof JDate)
                ev.end_date=ev.end_date._d;
            result.push(ev);
        }
	}
	return result;
...
scheduler.showEvent = function(id, mode) {
	...
	scheduler.setCurrentView(new  JDate(ev.start_date), mode);
...
scheduler._load = function(url, from) {
	...
	url += (url.indexOf("?") == -1 ? "?" : "&") + "timeshift=" + (new JDate()).getTimezoneOffset();
	...
	if (this._load_mode) {
		...
		from = this.date[this._load_mode + "_start"](new  JDate(from.valueOf()));
...
scheduler.ical={
	...
	parse_date:function(value,dh,dm){
		...
		if (scheduler.config.server_utc && !t[1]) { // if no hours/minutes were specified == full day event
			return new  JDate(Date.UTC(dy,dn,dd,dh,dm)) ;
		}
		return new  JDate(dy,dn,dd,dh,dm);
	},
...
scheduler.form_blocks={
	...
	time:{
		...
		//	debugger;
			...
			if(cfg.auto_end_date && cfg.event_duration) {
				var _update_lightbox_select = function () {
					start_date = new  JDate(s[map[3]].value,s[map[2]].value,s[map[1]].value,0,s[map[0]].value);
					end_date = new  JDate(start_date.getTime() + (scheduler.config.event_duration * 60 * 1000));
		...
		get_value:function(node, ev, config) {
			...
			ev.start_date=new  JDate(s[map[3]].value,s[map[2]].value,s[map[1]].value,0,s[map[0]].value);
			ev.end_date=new  JDate(s[map[3]+4].value,s[map[2]+4].value,s[map[1]+4].value,0,s[map[0]+4].value);
			...
			return {
				start_date: new  JDate(ev.start_date),
				end_date: new  JDate(ev.end_date)
			};
		},
		focus:function(node){
	...
	scheduler.attachEvent("onTemplatesReady", function() {
		...
		scheduler.templates.month_day = function(date) {
			if (this._mode == "month") {
				...
				if (+date == +scheduler.date.date_part(new JDate())) {

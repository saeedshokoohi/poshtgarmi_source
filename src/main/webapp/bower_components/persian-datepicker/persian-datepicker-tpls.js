...
angular.module('persian_datepicker', ['ui.bootstrap.dateparser', 'ui.bootstrap.position', 'ui.bootstrap.persian.datepicker.tpls', 'persianDate'])
...
//debugger;
        ...
        angular.forEach(['formatDay', 'formatMonth', 'formatYear', 'formatDayHeader', 'formatDayTitle', 'formatMonthTitle',
            'minMode', 'maxMode', 'showWeeks', 'startingDay', 'yearRange','offDateList'], function (key, index) {
        ...
        angular.forEach(['offDateList'], function (key) {
          //  debugger;
            if ($attrs[key]) {
        ...
        this.isDisabled = function (date) {
         //   console.log('minDate :'+this.minDate);
            return (this.isOffDate(date) || (this.minDate && this.compare(date, this.minDate) < 0) || (this.maxDate && this.compare(date, this.maxDate) > 0) || ($attrs.dateDisabled && $scope.dateDisabled({
                date: date,
                mode: $scope.datepickerMode
            })));
        };
        this.isOffDate = function (date) {
            //console.log(date.toLocaleDateString());
           var retVal=false;

            if(this.offDateList!=undefined) {

                var tempOffDateList = this.offDateList.split(',');
                angular.forEach(tempOffDateList,function(key,index){

                    var _d=new Date(key);
                    if(_d.toLocaleDateString()==date.toLocaleDateString())
                    {
                        retVal= true;
                        return;
                    }
            ...
            //if(retVal) console.log('off date found!!');
            return retVal;


        };
        this.getColor=function(date)
        {
            ...
            return {
                ...
                link: function (scope, element, attrs, ngModel) {
                   // debugger;
                    ...
                    scope.watchData = {};
                    angular.forEach(['minDate', 'maxDate', 'datepickerMode','offDateList'], function (key) {
                     //   debugger;
                        if (attrs[key]) {
...
angular.module("template/persianDatepicker/day.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/persianDatepicker/day.html",
        ...
        "        <button type=\"button\" style=\"width:100%;\" class=\"btn btn-default btn-sm\" ng-class=\"{'btn-info': dt.selected, active: isActive(dt)}\" ng-click=\"select(dt.date)\" ng-disabled=\"dt.disabled\" tabindex=\"-1\"><span ng-class=\"{'text-muted': dt.secondary, 'text-info': dt.current , 'offDate':dt.isOffDate}\">{{dt.label|EnToFaNumber}}</span></button>\n" +
        "      </td>\n" +
        "    </tr>\n" +
        "  </tbody>\n" +
        "</table>\n" +

        ...
        .directive('ngImageGallery', ['$rootScope', '$timeout', '$q', 'ngImageGalleryOpts',
            function ($rootScope, $timeout, $q, ngImageGalleryOpts) {
                return {
                    ...
                    '<div  style="float:left" ng-repeat="image in images">' +
                    '<table><tr><td>' +
                    '<div class="thumb"  ng-click="methods.open($index);" ng-style="{ \'background-image\': \'url(\' + (image.thumbUrl || image.url) + \')\' }"  ng-attr-title="{{image.title || undefined}}">' +
                    '</div>' +
                    '<td></tr><tr><td>' +
//remove btn
                    '<button class=" btn btn-default image-delete-btn" type="button" ng-click="methods.delete($index)">' +
